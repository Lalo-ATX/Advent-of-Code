...
#    print(f"testing {new_bookend.low_end}-{new_bookend.high_end} against {range_db}")
    ...
    # if we get here, that means we're #6, above, above all other entries
...
def ranges_counts(range_db: list[Bookends]) -> int:
    count = 0
    for check_range in range_db:
        count += check_range.high_end - check_range.low_end + 1
...
# now just count the size of each range
total_ids = ranges_counts(range_db)
print(f"total fresh IDs: {total_ids}")